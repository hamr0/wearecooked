...
// --- Delete a single cookie (Firefox API) ---
async function deleteCookie(cookie) {
  var protocol = cookie.secure ? "https" : "http";
  var domain = cookie.domain.startsWith(".") ? cookie.domain.slice(1) : cookie.domain;
  var url = protocol + "://" + domain + cookie.path;
  return browser.cookies.remove({ url: url, name: cookie.name, storeId: cookie.storeId });
}

// --- Render ---
function renderCleaner(cookies) {
  allCookies = cookies;
  var fp = deriveFirstPartyDomains(cookies);
  ...
  var catOrder = Object.entries(groups).sort(function(a, b) {
    var aRisky = RISKY_CATEGORIES.has(a[0]) ? 0 : 1;
    var bRisky = RISKY_CATEGORIES.has(b[0]) ? 0 : 1;
  ...
  selectedIds.clear();
  for (var i = 0; i < cookies.length; i++) {
    if (RISKY_CATEGORIES.has(cookies[i].category)) selectedIds.add(cookies[i]._idx);
  }
  ...
  html += '<div class="footer">wearecooked &middot; Cookie Cleaner</div>';
...
function bindEvents() {
  ...
  for (var i = 0; i < headers.length; i++) {
    headers[i].addEventListener("click", function(e) {
      ...
      document.getElementById("cat-body-" + catId).classList.toggle("open");
    });
  }
  ...
  for (var i = 0; i < catCbs.length; i++) {
    catCbs[i].addEventListener("change", function(e) {
      e.stopPropagation();
      var catId = this.getAttribute("data-catid");
      var checkboxes = document.getElementById("cat-body-" + catId).querySelectorAll(".cookie-checkbox");
...
// --- Init (Firefox: permissions granted at install, no button needed) ---
